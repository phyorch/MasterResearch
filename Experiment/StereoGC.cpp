...
void icvInitStereoTabs( CvStereoGCState2* state2 )
{
    int i, K = state2->K;
...
int icvGCResizeOrphansBuf( GCVtx**& orphans, int norphans )
{
...
int64 icvGCMaxFlow( GCVtx* vtx, int nvtx, GCEdge* edges, GCVtx**& _orphans, int& _maxOrphans )
{
...
inline int icvDataCostFuncGraySubpix( const uchar* a, const uchar* b )
{
    int va = a[0], vb = b[0];
...
inline int icvSmoothnessCostFunc( int da, int db, int maxR, const int* stabR, int scale )
{
    return da == db ? 0 : (da == OCCLUDED || db == OCCLUDED ? maxR : stabR[da - db])*scale;
}

void icvInitGraySubpix( const CvMat* left, const CvMat* right,
...
int64 icvComputeEnergy( const CvStereoGCState* state, const CvStereoGCState2* state2,
                               bool allOccluded )
{
...
inline void icvAddEdge( GCVtx *x, GCVtx* y, GCEdge* edgeBuf, int nedges, int w, int rw )
{
...
inline int icvAddTWeights( GCVtx* vtx, int sourceWeight, int sinkWeight )
{
    int w = vtx->weight;
    if( w > 0 )
        sourceWeight += w;
...
inline int icvAddTerm( GCVtx* x, GCVtx* y, int A, int B, int C, int D,
...
int64 icvAlphaExpand( int64 Eprev, int alpha, CvStereoGCState* state, CvStereoGCState2* state2 )
{
    GCVtx *var, *var1;
    int64 E = 0;
...
void cvFindStereoCorrespondenceGC( const CvArr* _left, const CvArr* _right,
